...
pub fn load_for_prompt_bytes(
    ...
    IMAGE_CACHE.get_or_try_insert_with(key, move || {
        ...
        let resize_options = match mode {
            PromptImageMode::Original => None,
            PromptImageMode::ResizeToFit => Some((MAX_DIMENSION, DEFAULT_JPEG_QUALITY)),
            PromptImageMode::ResizeToFitWith {
                max_dimension,
                jpeg_quality,
            } => Some((max_dimension.max(1), jpeg_quality.clamp(1, 100))),
        };

        let encoded = if resize_options
            .is_none_or(|(max_dimension, _)| width <= max_dimension && height <= max_dimension)
        {
            ...
            let (max_dimension, jpeg_quality) =
                resize_options.expect("resize options expected for oversized image");
            ...
            let (bytes, output_format) =
                encode_image_with_jpeg_quality(&resized, target_format, jpeg_quality)?;
            let mime = format_to_mime(output_format);
            EncodedImage {
                bytes,
                mime,
...
fn encode_image(
    ...
    encode_image_with_jpeg_quality(image, preferred_format, DEFAULT_JPEG_QUALITY)
}

fn encode_image_with_jpeg_quality(
    image: &DynamicImage,
    preferred_format: ImageFormat,
    jpeg_quality: u8,
) -> Result<(Vec<u8>, ImageFormat), ImageProcessingError> {
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn downscales_with_custom_bounds() {
        let image = ImageBuffer::from_pixel(1510, 749, Rgba([200u8, 10, 10, 255]));
        ...
        assert_eq!(processed.height, 595);
