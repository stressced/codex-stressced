...
const STRESSCED_REPEATED_TOOL_CALL_LIMIT: usize = 3;
...
pub(crate) struct ToolCallRuntime {
    ...
    repeated_tool_calls: Option<Arc<Mutex<HashMap<ToolCallFingerprint, usize>>>>,
}
...
impl ToolCallRuntime {
    pub(crate) fn new(
        ...
        Self {
            ...
            repeated_tool_calls: std::env::var("CODEX_STRESSCED_MODE")
                .is_ok()
    ...
    pub(crate) fn handle_tool_call_with_source(
        ...
        let guard_error = self.repeated_tool_call_guard(&call).err();
        ...
        let handle = guard_error.is_none().then(|| {
            ...
            let mut handle = match handle {
                ...
                None => {
                    ...
                    return Err(FunctionCallError::Fatal(
                        "tool dispatch skipped without a guard error".to_string(),
                    ));
                }
            };
...
impl ToolCallRuntime {
    fn repeated_tool_call_guard(&self, call: &ToolCall) -> Result<(), FunctionCallError> {
        let Some(repeated_tool_calls) = &self.repeated_tool_calls else {
            return Ok(());
        };
...
mod tests {
    ...
    #[tokio::test]
    async fn stressced_repeated_tool_call_guard_blocks_fourth_identical_call() -> anyhow::Result<()>
    {
        ...
        for index in 1..=STRESSCED_REPEATED_TOOL_CALL_LIMIT {
