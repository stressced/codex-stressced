...
fn build_content_for_model(exec_output: &ExecToolCallOutput) -> String {
    let output = &exec_output.aggregated_output.text;
    let content = if looks_like_binary_output(output) {
        format!(
            "{BINARY_OUTPUT_NOTICE}\nOriginal output length: {} bytes after decoding.",
            output.len()
        )
    } else {
        output.clone()
    ...
    build_content_with_optional_timeout(exec_output, content)
}

fn looks_like_binary_output(text: &str) -> bool {
    if text.is_empty() {
        return false;
    }
    ...
    let mut checked_chars = 0usize;
    let mut suspicious_chars = 0usize;

    for ch in text.chars().take(BINARY_OUTPUT_SAMPLE_CHARS) {
        checked_chars += 1;
...
mod tests {
    ...
    #[test]
    fn format_exec_output_omits_binary_like_output() {
        let formatted = format_exec_output_for_model(
            &exec_output("\u{1}\0\0\0vmf"),
            TruncationPolicy::Tokens(200),
        );

        assert_eq!(
            formatted,
            "Exit code: 0\nWall time: 0 seconds\nOutput:\nOutput omitted: command output appears to be binary or encoded data and was not included to protect the model context. Inspect it with metadata, hashes, Format-Hex, or a targeted decoder instead of printing raw contents.\nOriginal output length: 7 bytes after decoding."
        );
    }

    #[test]
    fn format_exec_output_keeps_text_output() {
        let formatted = format_exec_output_for_model(
            &exec_output("hello\nworld\n"),
            TruncationPolicy::Tokens(200),
        );
