...
const STRESSCED_VIEW_IMAGE_MAX_DIMENSION: u32 = 1200;
const STRESSCED_VIEW_IMAGE_JPEG_QUALITY: u8 = 80;
...
fn normalize_view_image_path(path: &str) -> &str {
...
impl ToolExecutor<ToolInvocation> for ViewImageHandler {
    ...
    async fn handle(
        ...
        let abs_path = cwd.join(normalize_view_image_path(&path));
        ...
        } else if is_stressced {
            PromptImageMode::ResizeToFitWith {
                max_dimension: STRESSCED_VIEW_IMAGE_MAX_DIMENSION,
                jpeg_quality: STRESSCED_VIEW_IMAGE_JPEG_QUALITY,
            }
        } else {
        ...
        let source_path = event_path.display().to_string();
    ...
    fn to_response_item(&self, call_id: &str, _payload: &ToolPayload) -> ResponseInputItem {
        let body = FunctionCallOutputBody::ContentItems(vec![
            FunctionCallOutputContentItem::InputText {
                text: format!(
                    "Image loaded from `{}` at {}x{}.",
                    self.source_path, self.width, self.height
                ),
            },
...
mod tests {
    ...
    async fn handle_strips_outer_quotes_from_path() {
        let (session, mut turn) = make_session_and_context().await;
        let image_dir = tempfile::tempdir().expect("create image temp dir");
        let image_cwd = image_dir.abs();
        ...
        let quoted_path = format!("\"{}\"", image_path.display());
        let result = ViewImageHandler::default()
            .handle(ToolInvocation {
                session: Arc::new(session),
                turn: Arc::new(turn),
                ...
                source: ToolCallSource::Direct,
                payload: ToolPayload::Function {
                    arguments: json!({ "path": quoted_path }).to_string(),
                },
            })
            .await;
        ...
        assert!(
            message.contains("unable to process image"),
            "quoted absolute path should locate the file before image decoding fails: {message}"
        );
        assert!(
            !message.contains(&quoted_path),
            "resolved path should not preserve outer quotes: {message}"
        );
    }
