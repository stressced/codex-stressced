...
use crate::tools::handlers::stressced_edit_guard::StresscedEditGuard;
...
impl ToolExecutor<ToolInvocation> for ExecCommandHandler {
    ...
    async fn handle(
        ...
        match manager
            ...
            .await
        {
            Ok(response) => Ok(boxed_tool_output(append_stressced_edit_notice(
                response,
                stressced_edit_guard,
            ))),
            Err(UnifiedExecError::SandboxDenied { output, .. }) => {
                ...
                Ok(boxed_tool_output(append_stressced_edit_notice(
                    response,
                    stressced_edit_guard,
                )))
            }
...
fn append_stressced_edit_notice(
    mut response: ExecCommandToolOutput,
    guard: Option<StresscedEditGuard>,
) -> ExecCommandToolOutput {
    let Some(notice) = guard.and_then(StresscedEditGuard::finish) else {
        return response;
    };

    if !response.raw_output.is_empty() && !response.raw_output.ends_with(b"\n") {
        response.raw_output.push(b'\n');
    }
    if !response.raw_output.is_empty() {
        response.raw_output.push(b'\n');
    }
    response.raw_output.extend_from_slice(notice.as_bytes());
    if response.original_token_count.is_some() {
        let text = String::from_utf8_lossy(&response.raw_output);
