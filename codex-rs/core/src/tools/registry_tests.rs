...
#[test]
fn handler_registers_flat_alias_for_namespaced_tools() {
    let namespaced_name = codex_tools::ToolName::namespaced("mcp__search", "lookup");
    let namespaced_handler = Arc::new(TestHandler {
        tool_name: namespaced_name,
    }) as Arc<dyn CoreToolRuntime>;
    let registry = ToolRegistry::from_tools([Arc::clone(&namespaced_handler)]);

    let alias = codex_tools::ToolName::plain("mcp__search__lookup");
    let tool = registry.tool(&alias);
