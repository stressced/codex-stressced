...
#[test]
fn stressced_adapts_tool_output_images_to_followup_message() {
    let image_url = "data:image/jpeg;base64,AAA".to_string();
    ...
    super::adapt_stressced_tool_output_images(&mut input);
    ...
    assert_eq!(
        ...
        vec![
            ResponseItem::FunctionCallOutput {
                call_id: "call-1".to_string(),
                output: FunctionCallOutputPayload {
                    body: FunctionCallOutputBody::Text("Image loaded by view_image.".to_string()),
                    success: Some(true),
                },
            },
            ResponseItem::Message {
...
#[test]
fn stressced_tools_remove_approval_controls() {
    let mut tools = vec![
        json!({
            "type": "function",
            "name": "shell_command",
        ...
        json!({
            "type": "namespace",
            "name": "mcp__tavily_mcp",
            "description": "Tavily MCP tools.",
            "tools": [
                {
                    "type": "function",
                    "name": "tavily_search",
    ...
    super::adapt_stressced_tools(&mut tools);
    ...
    let tavily_tool = tools
        .iter()
        .find(|tool| tool["name"] == "mcp__tavily_mcp__tavily_search")
        .expect("flattened tavily MCP tool");
    assert_eq!(tavily_tool["type"], "function");
    assert!(
        tavily_tool["description"]
            .as_str()
            .expect("description")
            .contains("Tavily MCP tools.")
    );
}

#[test]
fn stressced_recovers_textual_shell_tool_call() -> anyhow::Result<()> {
    let item = output_message(
        "msg-1",
        "I'll inspect it.\n\n<function=shell_command>\n<parameter=command>\nGet-ChildItem -Recurse -Path D:\\timeres\n</parameter>\n</function>\n</tool_call>",
    );

    let item = super::adapt_stressced_textual_tool_call(item);
    ...
    let ResponseItem::FunctionCall {
        ...
        panic!("expected recovered function call");
    };

    assert_eq!(name, "shell_command");
    assert!(call_id.starts_with("stressced_textual_shell_call_"));
    assert_eq!(
        serde_json::from_str::<serde_json::Value>(&arguments)?,
        json!({"command": "Get-ChildItem -Recurse -Path D:\\timeres"})
    );
