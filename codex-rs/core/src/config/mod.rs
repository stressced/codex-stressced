...
const STRESSCED_DEFAULT_MODEL_CONTEXT_WINDOW: i64 = 128_000;
const STRESSCED_DEFAULT_TOOL_OUTPUT_TOKEN_LIMIT: usize = 1_000;
...
impl Config {
    ...
    pub(crate) async fn load_config_with_layer_stack(
        ...
        if is_stressced {
            built_in.insert("llama_cpp".to_string(), ModelProviderInfo {
        ...
        let model_providers =
            merge_configured_model_providers(built_in, cfg.model_providers)
                .map_err(|message| std::io::Error::new(std::io::ErrorKind::InvalidData, message))?;

        let model_provider_id = {
            if is_stressced {
                let default_provider = "llama_cpp".to_string();
                if let Some(configured_provider) = &cfg.model_provider
                    && configured_provider != "llama_cpp" {
                        return Err(std::io::Error::new(
                            std::io::ErrorKind::InvalidData,
                            format!("codexstressced mode only supports llama_cpp provider. Configured provider '{configured_provider}' is not allowed."),
                        ));
                    }
                default_provider
            } else {
        ...
        let model = model.or(cfg.model).or_else(|| {
            if is_stressced && model_provider_id == "llama_cpp" {
                None // llama.cpp models change based on what is loaded, don't hardcode a default
            } else if model_provider_id == codex_model_provider_info::OLLAMA_OSS_PROVIDER_ID {
        ...
        let model_context_window = if is_stressced {
            cfg.model_context_window
                .or(Some(STRESSCED_DEFAULT_MODEL_CONTEXT_WINDOW))
        } else {
            cfg.model_context_window
        };
        let model_auto_compact_token_limit = if is_stressced {
            cfg.model_auto_compact_token_limit.or_else(|| {
                model_context_window.map(|context_window| context_window.saturating_mul(3) / 4)
            })
        } else {
            cfg.model_auto_compact_token_limit
        };
        let tool_output_token_limit = if is_stressced {
            cfg.tool_output_token_limit
                .or(Some(STRESSCED_DEFAULT_TOOL_OUTPUT_TOKEN_LIMIT))
        } else {
            cfg.tool_output_token_limit
        };
        let config = Self {
