...
//   - an array of structured content items (OpenAI cloud API)
//   - a plain string (required by llama.cpp and other local servers)
//
// We always serialize as a plain string for maximum compatibility.
// llama.cpp rejects structured content items with:
//   "Output of tool call should be 'Input text'"
// and the rejected entry poisons the session history, making all
// subsequent requests fail as well.
        ...
        S: Serializer,
    {
        match &self.body {
            ...
            FunctionCallOutputBody::ContentItems(items) => {
                // Flatten structured content items to plain text for
                // compatibility with servers that only accept strings.
                let text = function_call_output_content_items_to_text(items).unwrap_or_default();
