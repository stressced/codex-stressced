...
use codex_cli::codexstressced;
use codex_cli::llama_cmd::LlamaCommand;
...
struct MultitoolCli {
    /// The directory to use for configuration and data storage.
    /// When running as codexstressced, this overrides the default ~/.llmlocal directory.
    #[arg(long = "config-dir", env = "CODEX_CONFIG_DIR", value_name = "PATH")]
...
enum Subcommand {
    ...
    /// [codexstressced] Manage Llama models for local AI inference.
    #[clap(alias = "model")]
    Llama(LlamaCommand),
...
async fn run_llama_command(llama_cmd: LlamaCommand) -> anyhow::Result<()> {
    match llama_cmd.subcommand {
        codex_cli::llama_cmd::LlamaSubcommand::List => {
            let models = codex_cli::llama_cmd::list_models().await?;
            if models.is_empty() {
                eprintln!("No models found in Llama.cpp server. Please start a model first.");
            } else {
                eprintln!("Available Llama.cpp models:");
                for model in models {
                    eprintln!("  - {} (created: {})", model.id, model.created);
                }
            }
        }
        codex_cli::llama_cmd::LlamaSubcommand::Set { model } => {
            codex_cli::llama_cmd::set_selected_model(&model)?;
            eprintln!("✅ Default model set to: {}", model);
        }
        codex_cli::llama_cmd::LlamaSubcommand::Current => {
            let current = codex_cli::llama_cmd::get_selected_model()?;
            eprintln!("Current model: {}", current);
        }
        codex_cli::llama_cmd::LlamaSubcommand::Status => {
            let endpoint = codex_cli::llama_cmd::get_llama_endpoint();
            eprintln!("Llama.cpp endpoint: {}", endpoint);
            eprintln!("Status: Checking connectivity...");

            // Try to connect to Llama.cpp
                ...
                .await
            {
                Ok(response) => {
                    if response.status().is_success() {
                        eprintln!("✅ Connected to Llama.cpp server successfully");
                    } else {
                        eprintln!(
                            "❌ Failed to connect to Llama.cpp server: HTTP {}",
                            response.status()
                        );
                    }
                }
                Err(e) => {
                    eprintln!("❌ Failed to connect to Llama.cpp server: {}", e);
                }
            }
        }
    }
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    // Apply codexstressced-specific configuration if running in that mode
    if codexstressced::is_running_as_codexstressced() {
        codexstressced::print_banner();
        // We don't pass config_dir here since it's not available in this scope
        // The config_dir will be handled in the setup_isolation function
        codexstressced::setup_isolation(None)?;
    }
    ...
    // Apply provider restrictions if running in codexstressced mode
    if codexstressced::is_running_as_codexstressced() {
        // This would be where we would apply restrictions to the config
        // For now, we just ensure we're in the right mode
    }
    ...
    match subcommand {
        ...
        Some(Subcommand::Llama(llama_cmd)) => {
            reject_remote_mode_for_subcommand(
                root_remote.as_deref(),
                root_remote_auth_token_env.as_deref(),
                "llama",
            )?;
            run_llama_command(llama_cmd).await?;
        }
...
fn unsupported_subcommand_name_for_strict_config(
    ...
    match subcommand {
        ...
        Some(Subcommand::Llama(_)) => Some("llama"),
