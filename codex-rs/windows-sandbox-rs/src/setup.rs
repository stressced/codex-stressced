...
mod tests {
    ...
    fn payload_deny_write_paths_merge_explicit_and_protected_children() {
        ...
        let extra_codex = extra_write_root.join(".llmlocal");
        let explicit_deny = tmp.path().join("explicit-deny");
        fs::create_dir_all(&command_git).expect("create command .git");
        fs::create_dir_all(&extra_codex).expect("create extra .llmlocal");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![
        ...
        assert_eq!(
            [
                ...
                dunce::canonicalize(&extra_codex).expect("canonical extra .llmlocal"),
                explicit_deny,
            ]
            .into_iter()
